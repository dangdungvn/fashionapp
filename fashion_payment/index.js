        ...
        stripe.customers
          ...
          .then(async (customer) => {
            try {
              ...
              await createOrder(
                payload,
                customer.metadata.accesstoken,
                customer.metadata.fcm
              );
            } catch (err) {
...
const createOrder = async (payload, accessToken, fcm) => {
  try {
    const response = await fetch(
      "https://positively-genuine-lioness.ngrok-free.app/api/orders/add",
      {
        method: "POST",
        headers: {
    ...
    if (response.status === 201) {
      ...
      const message =
        "Your payment has been successfully completed and your order is now being processed";
      // if (fcm !== "") {
